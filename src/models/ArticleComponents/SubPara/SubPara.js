import "./SubPara.css";
const SubPara = ({ subtitle, subParaImg, subContent }) => {
  const hasImage = subParaImg !== undefined && subParaImg !== null;
  const containerClass = hasImage ? "sub-para with-image" : "sub-para without-image";
    ...
    if (Array.isArray(content)) {
      return content.map((sentence, index) => {
        if (typeof sentence === "string") {
          ...
          return <span key={index} dangerouslySetInnerHTML={{ __html: formattedSentence }} />;
        } else {
  ...
  return (
    <div className={containerClass}>
      ...
      {hasImage && <img className="sub-para-img-box-icon" alt="" src={subParaImg} />}
