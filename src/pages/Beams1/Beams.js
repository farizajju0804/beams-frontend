...
function Beams() {
  ...
  useEffect(() => {
    if (user) {
      ...
      fetch(`${API}/trending-cards?populate=*`)
        .then((res) => res.json())
        .then((trending) => {
          ...
          const mapData = trending.data
            ...
            .map((el, index) => {
              if (user.role.name === "admin")
                return { ...el.attributes, show: true };
              if (index <= showContents || index === 0)
                return { ...el.attributes, show: true };
              else
                return {
                  ...el.attributes,
                  ...
                  remainingTime: Math.floor(
                    (index * 259200000 - timeDifference) / (3600 * 1000)
                  ),
                };
            });
          setArticles(
            mapData.map((el) => {
              return {
                img: el.trendingCardImg,
                idofbeam: el.articleId,
                title: el.trendingCardTitle,
              };
            })
          );
          console.log(mapData);
